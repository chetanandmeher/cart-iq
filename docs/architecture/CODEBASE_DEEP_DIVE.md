...
| **Frontend** | React 19 + TypeScript + Vite + Recharts + Framer Motion | Real-time analytics dashboard |
...
| `SimulatorStatus` | `is_running`, `events_per_second` | Simulator status API |
| `SimulatorResponse` | `status`, `message` | Simulator start/stop APIs |
...
| `resetting` | `boolean` | State flag for active database/Redis reset operation |
| `resetMsg` | `string \| null` | Temporary overlay/banner message on successful reset |
...
> *This document was generated by analyzing every source file in the CartIQ repository. Last updated: May 26, 2026.*
